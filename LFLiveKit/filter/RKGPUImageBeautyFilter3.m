//
//  RKGPUImageBeautyFilter3.m
//  LFLiveKit
//
//  Created by Ken Sun on 2017/12/21.
//  Copyright © 2017年 admin. All rights reserved.
//

#import "RKGPUImageBeautyFilter3.h"

NSString *const kRKGPUImageBeauty3FragmentShaderString = SHADER_STRING
(
 ...
 void main() {
     ...
     for (int i = 0; i < 25; i++) {
         ...
         float w = i == 0 ? 28.0 : i < 10 ? 3.0 : 2.0;
         w = w / 84.0;
         a += w * (1.0 - abs(xi - x1) / (2.5 * threshold)) * xi;
         b += w * (1.0 - abs(xi - x1) / (2.5 * threshold));
     }
...
@implementation RKGPUImageBeautyFilter3

- (instancetype)init {
    if (self = [super initWithFragmentShaderFromString:kRKGPUImageBeauty3FragmentShaderString]) {
        [self setY:16.0];
